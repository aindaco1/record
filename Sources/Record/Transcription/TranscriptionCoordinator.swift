...
actor TranscriptionCoordinator {
    ...
    func resumePending(root: URL) {
        ...
        if !recovery.interrupted.isEmpty || !recovery.failed.isEmpty {
            let message = "recovered \(recovery.interrupted.count) interrupted and marked "
                + "\(recovery.failed.count) empty session(s) failed\n"
            FileHandle.standardError.write(Data(message.utf8))
        }
        for failure in recovery.errors {
            let message = "warning: could not recover "
                + "\(failure.directory.lastPathComponent): \(failure.description)\n"
            FileHandle.standardError.write(Data(message.utf8))
        }
