...
/// Manage lyrebird's LaunchAgent so the daemon starts at login.
///
/// We deliberately do NOT use SMAppService.mainApp here — that requires a full
/// .app bundle. Since lyrebird ships as a single binary in /usr/local/bin, a
...
struct Install: ParsableCommand {
    ...
    @Flag(name: .long, help: "Register lyrebird to start at login.")
    ...
    private static let label = "com.digimata.lyrebird"
    ...
    private func writeAgent() throws {
        ...
        let plist: [String: Any] = [
            ...
            "StandardOutPath": "/tmp/lyrebird.out.log",
            "StandardErrorPath": "/tmp/lyrebird.err.log",
        ]
        ...
        print("  binary: \(binary)")
        print("  logs:   /tmp/lyrebird.out.log, /tmp/lyrebird.err.log")
    }
    ...
    private func resolveBinaryPath() throws -> String {
        // /usr/local/bin/lyrebird is the canonical install path. Honor a real
        // location if running from elsewhere (e.g. dev).
        let candidate = "/usr/local/bin/lyrebird"
        if FileManager.default.isExecutableFile(atPath: candidate) {
            return candidate
        }
        // Fall back to the running executable's resolved path.
        let argv0 = CommandLine.arguments.first ?? "lyrebird"
        if argv0.hasPrefix("/"), FileManager.default.isExecutableFile(atPath: argv0) {
            FileHandle.standardError.write(Data(
                "note: /usr/local/bin/lyrebird not found; using \(argv0)\n".utf8
            ))
            return argv0
        }
        FileHandle.standardError.write(Data(
            "couldn't locate the lyrebird binary. install it to /usr/local/bin/lyrebird first.\n".utf8
        ))
        throw ExitCode(1)
    }
